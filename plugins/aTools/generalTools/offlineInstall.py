...
def aToolsOfflineInstall(offlineFilePath):
    ...
    zfobj.close()
    if os.path.isfile(tmpZipFile):     os.remove(tmpZipFile)
    from Framework.plugins.aTools import setup; reload(setup); setup.install([offlineFilePath, False])
    cmds.evalDeferred(\"from Framework.plugins.aTools.animTools.animBar import animBarUI; reload(animBarUI); animBarUI.show(\'refresh\')\")
