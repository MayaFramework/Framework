...
class SceneLoaderUI(form, base):
    ...
    def __populate_scenes(self, scenes_tree):
        ...
        for folder, scenes in scenes_tree.iteritems():
            ...
            scenes_list_widget.addItems(sorted([item for item in scenes.keys() if item.endswith(".ma")], reverse=True))
    ...
    def __scene_selected(self, scene):
        self.metadata_object = self.__generate_metadata_path_from_selection(scene.text().split(".")[0])
